    ...
    for i in range(data_count):
        ...
        if not pages:
            ...
            if not pages:
                ...
                surround_text = []
                for page in pages:
                    req_tag = requests.get("https:"+page.get("href"))
            ...
            for page in pages:
                ...
                if "https:" not in href:
                    req_tag = requests.get("https:"+href)
                else:
                    req_tag = requests.get(href)
                soup_tag = BeautifulSoup(req_tag.text, "lxml")
                links = soup_tag.select('div.list-berita a')
                for link in links:
                    href_link = link.get("href")
                    if "https:" not in href_link:
                        req_link = requests.get("https:"+href_link)
                    else:
                        req_link = requests.get(href_link)
...
file = "../dataset/data_training/kontan.csv" 
scrap = []
