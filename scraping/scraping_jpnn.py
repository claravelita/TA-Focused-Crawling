...
def AnchorText(file, scrap):
    ...
    for i in range(data_count):
        ...
        for s in soup.select('style'):
            s.extract()
        pages = soup.select('div.tags a')
        if not pages:
            pages = soup.select('div[itemprop~=articleBody] a')
            if not pages:
                pages = soup.select('div.terkait a')
                if not pages:
                    scrap.append("")
                else:
                    anchor_text = []
        ...
        if not pages:
            ...
            if not pages:
                ...
                for page in range(1, pages_count):
                    req_tag = requests.get(pages[page].get("href"))
                    soup_tag = BeautifulSoup(req_tag.text, "lxml")
                    links = soup_tag.select('div.content h1 a')
                    for link in links:
...
file = "../dataset/data_training/jpnn.csv" 
scrap = []
